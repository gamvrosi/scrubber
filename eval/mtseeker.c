...
int tnum = 1; /* Number of worker threads */
int seed = 0;
int count = 0;
int runtime = 0;
...
static void usage()
{
	fprintf(stderr, "Usage: "
			...
			"  -T  --threads TNUM    Number of worker threads to be used (def 1)\n"
...
ssize_t segread(int devfd, void* b, size_t size, off_t pos, int direct)
{
...
void *pthread_segread(void *thread_data){ /* Add loop and state support */
	int res, devfd;
	off_t pos;
	char buffer[BLOCKNUM];

	/* Extract preloaded/stable thread data from struct */
	struct thread_data *data = (struct thread_data *) thread_data;
	int tid = data->tid;

	printf("Thread %d has PID: %d, GID: %d\n", tid, (int) getpid(), (int) getgid());

//FIX: 	/* Set the I/O scheduling class and priority for the scrubber */
	res = ioprio_set(IOPRIO_WHO_PROCESS, (int) getpid(),
			 IOPRIO_PRIO_VALUE(IOPRIO_CLASS_IDLE,0));

	if (res < 0){
		fprintf(stderr, "ioprio_set failed!(%d)\n", errno);
		return 1;
	}
	...
	while(1){
		...
		if (data->state != TDONE){
			pthread_mutex_lock(&barrier_mutex[tid]);
			pthread_cond_wait(&barrier_cv[tid], &barrier_mutex[tid]);
			pthread_mutex_unlock(&barrier_mutex[tid]);
		}
		if (verbose) printf("Thread No.%d passed barrier.\n", tid);

		/* Perform state-related actions */
		switch (data->state){
			case TDONE:
				if (verbose) printf("THREAD_%d: On TDONE\n",tid);
				pthread_exit(NULL);
			case TBUSY:
				if (verbose) printf("THREAD_%d: On TBUSY\n",tid);

				/* Extract data loaded in struct */
				devfd = data->devfd;
				pos = data->pos;

				/* Offset should be in *bytes*, just like size */
				res = segread(devfd, buffer, BLOCKNUM, pos * BLOCKNUM, direct);

				if (res)
					fprintf(stderr, "segread failed!(%d)\n", res);
				...
				break;
			case TIDLE:
				if (verbose) printf("THREAD_%d: On TIDLE\n",tid);
				/* How did we get here? */
			default:
				if (verbose) printf("THREAD_%d: On DEF\n",tid);
				break;
		}
	}
...
int main(int argc, char **argv)
{
	...
	if (sectorstart && sectorcount && (sectorstart + sectorcount) < numblocks)
	...
	if (workload == TESTCACHE) {
		...
		for (k = 0; k < 16384; k++)
			retval = segread(dev, buffer, sysconf(_SC_PAGESIZE),
					k * sysconf(_SC_PAGESIZE), 1);

		for (k = 0; k < 50; k++) {
			gettimeofday(&ra, NULL);
			retval = segread(dev, buffer, sysconf(_SC_PAGESIZE),
					testblks[k], 1);
		...
		for (k = 0; k < 50; k++) {
			gettimeofday(&ra, NULL);
			retval = segread(dev, buffer, sysconf(_SC_PAGESIZE),
					testblks[k] + sysconf(_SC_PAGESIZE), 1);
			gettimeofday(&rb, NULL);
		...
		for (;;) {
			...
			gettimeofday(&reqa, NULL);
			/* Convert offset from request no. to *bytes* and issue the read() */
			retval = segread(dev, buffer, BLOCKNUM, offset * BLOCKNUM, direct);
			gettimeofday(&reqb, NULL);
