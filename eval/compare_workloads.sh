...
filename="comparison"$timestamp""
mlabfile="comparison"$timestamp".m"
...
function run_synthetic {
	...
	for wl in $workloads; do
		...
		print_msg "$1"
		delay="-6 -5 -4 -3 -2 -1 0 8 16 32 64 128 256"
		...
		for sc in $scrubbers; do
			...
			echo -e "$wl${sc}_delay = {};" >> $mlabfile
			echo -e "$wl${sc}_stime = [0 0 0];" >> $mlabfile
			echo -e "$wl${sc}S_thput = [0 0 0];" >> $mlabfile
			echo -e "$wl${sc}S_rtime = [0 0 0];" >> $mlabfile
			echo -e "$wl${sc}S_count = [0 0 0];" >> $mlabfile
			...
			for d in $delay
			do
				echo -ne $wl$sc"_delay = ["$wl$sc"_delay '"$d"'];\n" >> $mlabfile
				if [ "$d" == "-6" ]; then
					# Run workload by itself for $runtime sec
					run_scrub "$wl by itself" $wl$sc $2 $3 $4 "NONE" "realtime" $wl "DEF" \
						0 $capacity $runtime 0.25 0 0 0 0 1 0 $d
				else
					if [ "$d" == "-5" ]; then
						# Run workload by itself for $runtime sec
						run_scrub "$wl by itself" $wl$sc $2 $3 $4 "NONE" "realtime" $wl "DEF" \
							0 $capacity $runtime 0.5 0 0 0 0 1 0 $d
					else
						if [ "$d" == "-4" ]; then
						# Run workload by itself for $runtime sec
						run_scrub "$wl by itself" $wl$sc $2 $3 $4 "NONE" "realtime" $wl "DEF" \
							0 $capacity $runtime 1.0 0 0 0 0 1 0 $d
						else
							if [ "$d" == "-3" ]; then
								# Run scrubber with CFQ priorities enabled
								run_scrub "$wl with CFQ prio ON" "$wl$sc" $2 $3 $4 $sc "idlechk" \
									$wl "DEF" 0 "$capacity" 0 0.25 "$parts" $scrubarea \
									$ss $rs 1 $reqno "$d"
							else
								if [ "$d" == "-2" ]; then
									# Run scrubber with CFQ priorities enabled
									run_scrub "$wl with CFQ prio ON" "$wl$sc" $2 $3 $4 $sc "idlechk" \
										$wl "DEF" 0 "$capacity" 0 0.5 "$parts" $scrubarea \
										$ss $rs 1 $reqno "$d"
								else
									if [ "$d" == "-1" ]; then
										# Run scrubber with CFQ priorities enabled
										run_scrub "$wl with CFQ prio ON" "$wl$sc" $2 $3 $4 $sc "idlechk" \
											$wl "DEF" 0 "$capacity" 0 1.0 "$parts" $scrubarea \
											$ss $rs 1 $reqno "$d"
									else
										# Run scrubber with workload
										run_scrub "$wl with ${d}ms delay" "$wl$sc" $2 $3 $4 $sc "realtime" \
											$wl "DEF" 0 "$capacity" 0 $thinktime "$parts" $scrubarea \
											$ss $rs 1 $reqno "$d"
									fi
								fi
							fi
